...
imports_2012_2024 <- imports_2012_2024 |>
  group_by(year) |>
  summarise(
...
imports_by_year_cpi <- imports_2012_2024 |>
  left_join(cpi,
            by = "year")

imports_inflation_adjusted <-
  mutate(imports_by_year_cpi,
         adj_dollars = (weighed_avg_import)*(cpi_now/avg_cpi))

imports_inflation_adjusted <-
  na.omit(imports_inflation_adjusted) # omit any missing values

imports_inflation_adjusted <-
  imports_inflation_adjusted[, c(1, 3, 5)]

## Export the file(s) ----------------------------------------------------------
write_rds(x = imports_inflation_adjusted,
